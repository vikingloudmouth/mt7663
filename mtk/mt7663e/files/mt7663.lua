...
function add_vif_into_lan(vif)
    ...
    -- end
    if not (string.match(vif, "ra1") or string.match(vif, "rai1"))  then 
        nixio.syslog("debug", "add "..vif.." into lan")
...
function mt7663_up(devname)
    ...
    if mtkwifi.exists("/sys/class/net/"..dev.main_ifname) then
        -- local main_dis = mtkwifi.__trim( mtkwifi.read_pipe("uci get wireless."..dev.main_ifname..".disabled"))
        -- if main_dis == "0" then
        ...
        add_vif_into_lan(dev.main_ifname)
            if wifi_services_exist then
                ...
                wps_pin(devname,dev.main_ifname,true)
                HwNat_enable(devname,dev.main_ifname)
            end
        -- end
    else
    ...
    do
        ...
        then
            -- local vif_dis = mtkwifi.__trim( mtkwifi.read_pipe("uci get wireless."..vif..".disabled"))
            -- if vif_dis == "0" then 
