...
typedef struct _EAP_AGG_LIMIT_SET {
	UINT_8 AALCRIdx;
	UINT_8 Count;
	UINT_8 aucReserved[2];
} EAP_AGG_LIMIT_SET;

typedef struct _CMD_SET_AGG_LIMIT_T {
	UINT32 u4EapCtrlCmdId;
	EAP_AGG_LIMIT_SET eap_agglimit;
	UINT_8  u1Reserved;
} CMD_SET_AGG_LIMIT_T, *P_CMD_SET_AGG_LIMIT_T;

INT Set_AMPDU_MPDU_Count(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_AMPDU_Retry_Count(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Show_AMPDU_MPDU_Count(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Show_AMPDU_retry_Count(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
#endif /* AMPDU_CONF_SUPPORT */

#ifdef ACK_CTS_TIMEOUT_SUPPORT

typedef struct _EAP_ACKTIMEOUT_SET {
	UINT32 bTimeout;
	UINT_8 ModuleMode;
	UINT_8 aucReserved[3];
} EAP_ACKTIMEOUT_SET;

typedef struct _CMD_SET_ACKTIMEOUT_T {
	UINT32 u4EapCtrlCmdId;
	EAP_ACKTIMEOUT_SET eap_acktimeout;
	UINT_8  u1Reserved;
} CMD_SET_ACKTIMEOUT_T, *P_CMD_SET_ACKTIMEOUT_T;

void Show_Distance_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Show_CCK_ACK_Timeout_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Show_OFDM_ACK_Timeout_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_CCK_ACK_Timeout_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_OFDM_ACK_Timeout_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
INT Set_DST_to_ACK_Timeout_Proc(struct _RTMP_ADAPTER *pAd, RTMP_STRING *arg);
...
#ifdef WIFI_EAP_FEATURE
#ifdef AMPDU_CONF_SUPPORT
INT32 CmdEapSetAGGLimit(
	struct _RTMP_ADAPTER *pAd,
	struct _EAP_AGG_LIMIT_SET *peap_agglimit,
	UINT32 u4RegValue
);
#endif /* AMPDU_CONF_SUPPORT */

#ifdef ACK_CTS_TIMEOUT_SUPPORT
INT32 CmdEapSetAckTimeout(
	struct _RTMP_ADAPTER *pAd,
	struct _EAP_ACKTIMEOUT_SET *peap_acktimeout
);
#endif /* ACK_CTS_TIMEOUT_SUPPORT */
#endif /* WIFI_EAP_FEATURE */
