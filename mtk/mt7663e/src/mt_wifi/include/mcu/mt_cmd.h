...
enum EXT_CMD_TYPE {
	...
	EXT_CMD_ID_GET_WTBL_TX_COUNTER = 0xA7,
#endif
...
enum EXT_EVENT_TYPE {
	...
	EXT_EVENT_ID_GET_WTBL_TX_COUNTER = 0xA7,
#endif
...
#ifdef TXRX_STAT_SUPPORT
typedef struct _PER_STA_TX_STAT_T {
	UINT32	u4PerStaTxPktCnt;
	UINT32	u4PerStaTxFailPktCnt;
	UINT32	u4PerStaTxRate1FailCount;
} PER_STA_TX_STAT_T, *P_PER_STA_TX_STAT_T;
...
typedef struct _EXT_EVENT_STA_TX_STAT_RESULT_T {
	PER_STA_TX_STAT_T arPerStaTxStats[MAX_LEN_OF_MAC_TABLE];
	UINT8	ucEntryBitmap[16];
	UINT8	ucEntryCount;
	UINT8	aucReserved[3];
} EXT_EVENT_STA_TX_STAT_RESULT_T, *P_EXT_EVENT_STA_TX_STAT_RESULT_T;
#endif
#ifdef CUSTOMER_DCC_FEATURE
...
#ifdef MIN_PHY_RATE_SUPPORT
INT32 MtCmdSetMinPhyRate(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT8 band);
#endif /* MIN_PHY_RATE_SUPPORT */
#endif /* RACTRL_FW_OFFLOAD_SUPPORT */
...
#ifdef WIFI_EAP_FEATURE
INT32 MtCmdSetEdccaThreshold(struct _RTMP_ADAPTER *pAd, UINT32 edcca_threshold, UINT8 BandIdx);
