					...
					UINT32 Band)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: %d %d %d %d\n",
				__func__, AntIndex, DcOffset_I, DcOffset_Q, Band));
...
static INT32 MT_ATETxCWTone(RTMP_ADAPTER *pAd, UINT32 Control)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Control = %d Channel=%d AntMask=0x%x\n",
								__func__, Control, Channel, AntMask));
	/*2.4G */
	if (Channel <= 14)
		Band = 2407000;
	else
	...
	for (AntIdx = 0; AntIdx <= 3; AntIdx++) {
		if (Control == 0) {
			MtCmdTxTonePower(pAd, RF_AT_EXT_FUNCID_TX_TONE_DIGITAL_GAIN, 0, AntIdx, band_idx);
			MtCmdTxTonePower(pAd, RF_AT_EXT_FUNCID_TX_TONE_RF_GAIN, 0, AntIdx, band_idx);
		} else {
	...
	MtCmdTxTone(pAd, band_idx, Control, AntMask, 0, 0, (Control == 1) ? DcOffset_I : 0, (Control == 1) ? DcOffset_Q : 0, Band);
...
#endif
			if (pAd->E2pAccessMode == E2P_BIN_MODE) {
				rtmp_cal_write_to_bin(pAd, pAd->PreCalImageInfo, 0, PRE_CAL_IMAGE_INFO_SIZE);
				rtmp_cal_write_to_bin(pAd, pAd->PreCalImage, PRE_CAL_BIN_OFFSET, PRE_CAL_TOTAL_SIZE);
...
#endif
		if (pAd->E2pAccessMode == E2P_BIN_MODE) {
			rtmp_cal_write_to_bin(pAd, pAd->PreCalImage, DPD_BIN_OFFSET, DPD_CAL_TOTAL_SIZE);
			rtmp_ee_bin_read16(pAd, 0x52, &doCal1);
