...
INT32 SetATERFPower(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	RFPower = os_str_tol(Arg, 0, 10);

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: RF_Power = %d, band_idx = %d \n",
							__func__, RFPower, band_idx));
...
INT32 SetATEDigitalPower(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	Digital_power = os_str_tol(Arg, 0, 10);

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Digital_Power = %d, band_idx = %d \n",
								__func__, Digital_power, band_idx));
...
INT32 SetATEDCOffset_I(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: DCOffset_I = %s\n", __func__, Arg));

	DCOffset_I = os_str_tol(Arg, 0, 10);

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: DCOffset_I = %d, band_idx = %d \n",
								__func__, DCOffset_I, band_idx));
...
INT32 SetATEDCOffset_Q(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: DCOffset_Q = %s\n", __func__, Arg));

	DCOffset_Q = os_str_tol(Arg, 0, 10);

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: DCOffset_Q = %d, band_idx = %d \n",
								__func__, DCOffset_Q, band_idx));
...
INT SetATETxBfGdInitProc(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[0], 0x00);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[1], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[2], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[3], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[4], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr1[5], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[0], 0x00);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[1], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[2], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[3], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[4], 0x22);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr1[5], 0x22);
	snprintf(cmdStr, sizeof(cmdStr), "00:%.2x:%.2x:%.2x:%.2x:%.2x",
		0x22, 0x22, 0x22, 0x22, 0x22);
	SetATEDa(pAd, cmdStr);
	/* set ATESA=00:11:11:11:11:11 */
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[0], 0x00);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[1], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[2], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[3], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[4], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Addr2[5], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[0], 0x00);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[1], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[2], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[3], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[4], 0x11);
	TESTMODE_SET_PARAM(ATECtrl, 0, Addr2[5], 0x11);
	...
	if (fgBw160) {
		...
		if (control_band_idx == 0) {
			...
			AsicDevInfoUpdate(
				pAd,
				IS_MT7626(pAd) ? HcGetOmacIdx(pAd, wdev):0x11,
				ATECtrl->Addr2,
				control_band_idx,
				TRUE,
				DEVINFO_ACTIVE_FEATURE);
	...
	if (fgBw160) {
		...
		if (control_band_idx == 0) {
			...
			if IS_MT7626(pAd)
				snprintf(cmdStr, sizeof(cmdStr), "%.2x:01:00:%.2x:%.2x:%.2x:%.2x:%.2x",
					HcGetOmacIdx(pAd, wdev), 0x22, 0x22, 0x22, 0x22, 0x22);
			else
				snprintf(cmdStr, sizeof(cmdStr), "11:01:00:%.2x:%.2x:%.2x:%.2x:%.2x",
					0x22, 0x22, 0x22, 0x22, 0x22);
		}
...
INT32 SetATEIBfProfileUpdate(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	/*remove Set_StaRecCmmUpdate because eBF have set it already*/
	...
	RTMP_STRING *Arg)
{
	...
	if (!strcmp(Arg, "ATESTART") && (mode != ATE_START)) { /* support restart w/o ATESTOP */
		...
		Ret = ATEOp->TxCWTone(pAd, 1);

	else if (!strcmp(Arg, "TXCWTONESTOP") && (mode & ATE_START))
		Ret = ATEOp->TxCWTone(pAd, 0);
#endif
