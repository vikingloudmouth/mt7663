...
static struct {
...
#ifdef ATE_CW_TONE_MT7663_SUPPORT
...
#ifdef CONFIG_MT7663_DPD_RE_CAL_SUPPORT
	{"DPDbufferWriteBack",			Set_DPDBufferWriteBack_Proc},
	{"PreCalWriteBack",			Set_PreCalBufferWriteBack_Proc},
#endif
	...
	*/
	...
	{"Distance",            Set_DST_to_ACK_Timeout_Proc},
	...
	{"vowmode", Set_vowMode_Manual_Proc},
#endif /*IXIA_SUPPORT*/
...
#define	ASSO_MAC_LINE_LEN	(1+19+4+4+4+4+8+7+7+7+12+9+12+9+10+7+10+7+1)
#else
#define	ASSO_MAC_LINE_LEN	(1+19+4+4+4+4+8+7+7+7+7+12+9+12+9+10+7+10+7+1)
#endif
#if defined(VENDOR_FEATURE6_SUPPORT) || defined(CUSTOMER_DCC_FEATURE)
...
#endif
	sprintf(msg + strlen(msg), "%-12s%-9s%-12s%-9s%-10s%-7s%-10s%-7s\n", "PhMd", "BW", "MCS", "SGI", "STBC", "Idle", "Rate", "TIME");
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
...
#endif /* DOT11_N_SUPPORT */
#ifdef CUSTOMER_DCC_FEATURE
			sprintf(msg + strlen(msg), "%-7d%-7d%-7d", pEntry->RssiSample.AvgRssi[0], pEntry->RssiSample.AvgRssi[1], pEntry->RssiSample.AvgRssi[2]);
#else
		...
		if (cap->fgRateAdaptFWOffload == TRUE && (pEntry->bAutoTxRateSwitch == TRUE)) {
			...
			if (pEntry->bAutoTxRateSwitch == TRUE) {
				...
				sprintf(msg + strlen(msg), "%-12s", tmp_str);
				snprintf(tmp_str, temp_str_len, "%s/%s", get_bw_str(bw), get_bw_str(bw_r));
				sprintf(msg + strlen(msg), "%-9s", tmp_str);
#ifdef DOT11_VHT_AC
				...
				sprintf(msg + strlen(msg), "%-10s", tmp_str);
...
#else
			sprintf(msg + strlen(msg), "%-12s", get_phymode_str(pEntry->HTPhyMode.field.MODE));
			sprintf(msg + strlen(msg), "%-9s", get_bw_str(pEntry->HTPhyMode.field.BW));

#ifdef DOT11_VHT_AC

			if (pEntry->HTPhyMode.field.MODE == MODE_VHT)
				sprintf(msg + strlen(msg), "%dS-M%-9d", ((pEntry->HTPhyMode.field.MCS >> 4) + 1), (pEntry->HTPhyMode.field.MCS & 0xf));
			else
#endif /* DOT11_VHT_AC */
				sprintf(msg + strlen(msg), "%-12d", pEntry->HTPhyMode.field.MCS);
				sprintf(msg + strlen(msg), "%-9d", pEntry->HTPhyMode.field.ShortGI);
				sprintf(msg + strlen(msg), "%-10d", pEntry->HTPhyMode.field.STBC);
				sprintf(msg + strlen(msg), "%-7d", (int)(pEntry->StaIdleTimeout - pEntry->NoDataIdleCount));
				sprintf(msg + strlen(msg), "%-10d", (int)DataRate);
#endif
	...
	IN INT cmd)
{
	...
	{
		int i = 0;
		struct msg_channel_list *Rsp = NULL;
		UCHAR BandIdx = 0;
		...
		Rsp = (struct msg_channel_list *)kmalloc(sizeof(struct msg_channel_list), MEM_ALLOC_FLAG);
		if (Rsp == NULL)
			return FALSE;
	...
	IN INT cmd)
{
	...
	case OID_GET_WSC_PROFILES:
		...
		case OID_GET_WMODE:
		{
			...
			if (wdev && (wdev->if_up_down_state == TRUE)) {
				wmode = wdev->PhyMode;
	...
	IN	RTMP_STRING *arg)
{
	...
	if ((DtimPeriod >= 1) && (DtimPeriod <= 255)) {
		...
		wdev->bss_info_argument.dtim_period = pAd->ApCfg.DtimPeriod;
		wdev->bss_info_argument.u4BssInfoFeature = BSS_INFO_BASIC_FEATURE;
		if (AsicBssInfoUpdate(pAd, &wdev->bss_info_argument) != NDIS_STATUS_SUCCESS)
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("Fail to apply the bssinfo\n"));
	...
	IN	RTMP_STRING *arg)
{
	...
	if ((value_interval >= 0) && (value_interval < MAX_GROUP_REKEY_INTERVAL))
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_AccessPolicy_Proc::(AccessPolicy=%ld)\n",
	...
	IN	RTMP_STRING *arg)
{
	...
	while ((this_char = strsep((char **)&arg, ";")) != NULL) {
		if (*this_char == '\0') {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("An unnecessary delimiter entered!\n"));
			continue;
		}
		...
		for (j = 0; j < pacl->Num; j++) {
			if (memcmp(pacl->Entry[j].Addr, &macAddr, 6) == 0) {
				isDuplicate = TRUE;
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("You have added an entry before :\n"));
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The duplicate entry is %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	IN	RTMP_STRING *arg)
{
	...
	while ((this_char = strsep((char **)&arg, ";")) != NULL) {
		...
		if (!isFound) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The entry %02x:%02x:%02x:%02x:%02x:%02x is not in the list!\n",
					 macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]));
		}
	}
	...
	/* check if the change in ACL affects any existent association */
	ApUpdateAccessControlList(pAd, pObj->ioctl_if);
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set::%s(Policy=%ld, Entry#=%ld)\n",
			 __func__, pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.Policy,
			 pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.Num));
#ifdef DBG
	...
	IN	RTMP_STRING *arg)
{
	...
	if (bDumpAll == 1)
		...
		bDumpAll = FALSE;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Your input is 0!\n"));
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The Access Control List will not be dumped!\n"));
		return TRUE;
	} else {
		return FALSE;  /* Invalid argument */
	}
	...
	if (acl.Num == 0) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The Access Control List is empty!\n"));
		return TRUE;
	}
	...
	IN	RTMP_STRING *arg)
{
	...
	if (bClearAll == 1)
		...
		bClearAll = FALSE;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Your input is 0!\n"));
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The Access Control List will be kept unchanged!\n"));
		return TRUE;
	} else {
		return FALSE;  /* Invalid argument */
	}
	...
	if (pacl->Num == 0) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The Access Control List is empty!\n"));
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("No need to clear the Access Control List!\n"));
...
INT Show_Wifi_Stats_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    if (!VALID_WCID(Wcid)) {
		...
		if (IS_VALID_ENTRY(pEntry) && IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("WCID\tMAC Address\t\tmpdu_attempts\tmpdu_retries\tlow rate fail count\n"));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%d\t%02X:%02X:%02X:%02X:%02X:%02X\t",
					  pEntry->wcid, PRINT_MAC(pEntry->Addr)));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("%10lld\t%10lld\t%10lld\n", pEntry->mpdu_attempts.QuadPart,
					pEntry->mpdu_retries.QuadPart, pEntry->mpdu_low_rate_fail_cnt.QuadPart));
		} else {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIFHWADDR:
		...
		if (pObj->ioctl_if < MAX_MBSSID_NUM(pAd)) {
			if (wrq->u.data.length >= 6) {
				copy_to_user(wrq->u.name, pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.bssid, 6);
			}
		}
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIWESSID: {
		...
		if (pSSID->priv_flags == INT_APCLI) {
			if (pObj->ioctl_if >= MAX_MULTI_STA) {
				...
				pConfig->Status = -EFAULT;
				Status = NDIS_STATUS_FAILURE;
				MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s -> CMD_RTPRIV_IOCTL_AP_SIOCGIWESSID fail\n", __func__));
				break;
			}
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIWAP: {
		...
		if (Data == INT_APCLI) {
			if (pObj->ioctl_if >= MAX_MULTI_STA) {
				...
				pConfig->Status = -EFAULT;
				Status = NDIS_STATUS_FAILURE;
				MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s -> CMD_RTPRIV_IOCTL_AP_SIOCGIWAP fail\n", __func__));
				break;
			}
			if (pAd->StaCfg[pObj->ioctl_if].ApcliInfStat.Valid == TRUE)
...
void ApSendConnFailMsg(PRTMP_ADAPTER pAd, UCHAR apidx, UCHAR *StaAddr, USHORT ReasonCode)
{
	struct CONN_FAIL_MSG msg;
	CHAR *Ssid = pAd->ApCfg.MBSSID[apidx].Ssid;
	UCHAR SsidLen = pAd->ApCfg.MBSSID[apidx].SsidLen;
