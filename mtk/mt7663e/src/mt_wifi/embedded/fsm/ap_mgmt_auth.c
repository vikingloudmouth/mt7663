	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	if (VALID_UCAST_ENTRY_WCID(pAd, Elem->Wcid)) {
		...
		if (IS_ENTRY_CLIENT(pEntry))
			ApSendConnFailMsg(pAd, pEntry->func_tb_idx, pEntry->Addr, pInfo->reason);
#endif
...
VOID ap_peer_auth_req_at_idle_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	auth_info = (AUTH_FRAME_INFO *)kmalloc(sizeof(AUTH_FRAME_INFO), MEM_ALLOC_FLAG);
	if (auth_info == NULL)
		return;
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry)) {
		...
		if (!RTMPEqualMemory(auth_info->addr1, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].wdev.bssid,
					MAC_ADDR_LEN)) {
...
SendAuth:
	...
	if ((pMbss->wdev.WscControl.WscConfMode != WSC_DISABLE) &&
		...
		if (!ApCheckAccessControlList(pAd, auth_info->addr2, apidx)) {
			...
			ASSERT(pEntry == NULL);
			ap_peer_auth_simple_rsp_gen_and_send(pAd, pRcvHdr, auth_info->auth_alg, auth_info->auth_seq + 1,
								MLME_UNSPECIFY_FAIL);
#ifdef WIFI_DIAG
			DiagConnError(pAd, apidx, auth_info->addr2, DIAG_CONN_ACL_BLK, 0);
#endif
#ifdef CONN_FAIL_EVENT
			ApSendConnFailMsg(pAd, apidx, auth_info->addr2, REASON_DECLINED);
#endif
...
#endif
#ifdef CONN_FAIL_EVENT
			ApSendConnFailMsg(pAd, apidx, auth_info->addr2, REASON_UNSPECIFY);
#endif
			 goto auth_failure;
...
#define	RADIUS_ACL_ACCEPT_TIMEOUT  3
		...
		if (pEntry != NULL) {
			...
			if (pFtInfoBuf) {
				...
				if (result == MLME_SUCCESS) {
					NdisMoveMemory(&pEntry->MdIeInfo, &auth_info->FtInfo.MdIeInfo,
							sizeof(FT_MDIE_INFO));
	...
	if ((auth_info->auth_alg == AUTH_MODE_SAE) &&
		...
		if (pmk) {
			...
			if (!pEntry)
				pEntry = MacTableInsertEntry(pAd, auth_info->addr2, wdev, ENTRY_CLIENT,
								OPMODE_AP, TRUE);
...
#endif
#ifdef CONN_FAIL_EVENT
				ApSendConnFailMsg(pAd, apidx, auth_info->addr2, REASON_DISASSOC_AP_UNABLE);
#endif
			}
		} else if ((auth_info->auth_alg == AUTH_MODE_KEY) &&
				   (IS_AKM_SHARED(pMbss->wdev.SecConfig.AKMMap)
...
#ifdef WIFI_DIAG
			if (IS_ENTRY_CLIENT(pEntry))
				DiagConnError(pAd, pEntry->func_tb_idx, pEntry->Addr, DIAG_CONN_AUTH_FAIL, REASON_AUTH_WRONG_ALGORITHM);
#endif
#ifdef CONN_FAIL_EVENT
			if (IS_ENTRY_CLIENT(pEntry))
				ApSendConnFailMsg(pAd, pEntry->func_tb_idx, pEntry->Addr, REASON_AKMP_NOT_VALID);
#endif
				MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
			}
...
#ifdef WIFI_DIAG
				if (IS_ENTRY_CLIENT(pEntry))
					DiagConnError(pAd, pEntry->func_tb_idx, pEntry->Addr, DIAG_CONN_AUTH_FAIL, REASON_CHALLENGE_FAIL);
#endif
#ifdef CONN_FAIL_EVENT
				if (IS_ENTRY_CLIENT(pEntry))
					ApSendConnFailMsg(pAd, pEntry->func_tb_idx, pEntry->Addr, REASON_AKMP_NOT_VALID);
...
#ifdef WIFI_DIAG
			if (IS_ENTRY_CLIENT(pEntry))
				DiagConnError(pAd, pEntry->func_tb_idx, pEntry->Addr, DIAG_CONN_AUTH_FAIL, REASON_UNKNOWN);
#endif
#ifdef CONN_FAIL_EVENT
			if (IS_ENTRY_CLIENT(pEntry))
				ApSendConnFailMsg(pAd, pEntry->func_tb_idx, pEntry->Addr, REASON_UNSPECIFY);
#endif
			MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
		}
	}
