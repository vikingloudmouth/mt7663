		...
		for (i = 0; VALID_UCAST_ENTRY_WCID(pAd, i); i++) {
			...
			if (pEntry && (MAC_ADDR_EQUAL(pEntry->Addr, wdev->bssid))
				...
				if (pEntry->one_sec_tx_succ_pkts > 70) {
					skip_20_40_scan = 1;
					MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!!!! skip_20_40_scan %lu !!!!!!\n",
						pEntry->one_sec_tx_succ_pkts));
					break;
				}
			MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Last_Sec_Tx_Count %lu Rept=%d\n",
