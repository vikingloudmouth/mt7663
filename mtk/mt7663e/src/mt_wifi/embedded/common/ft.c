	...
	IN UCHAR RsnIeLen)
{
	...
	if ((pRicInfo != NULL) && (pFtIeInfo != NULL) && pFtIeInfo->MICCtr.field.IECnt) {
			...
			if ((IS_AKM_FT_WPA2(pEntry->AKMMap))
				...
				*/
				...
				if (pMacEntry && pMacEntry->wdev) {
					cntl_disconnect_request(pMacEntry->wdev, CNTL_DISASSOC, pEntry->StaMac, REASON_DISASSOC_STA_LEAVING, NULL);
					/*
					...
					MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR,
							("%s: PMKCache timeout. Kick out the station wcid(%d)"
	...
	OUT		PUCHAR		pR0KH_Id)
{
	...
	*R0KH_IdLen = 0;
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse (  FtIeLen = %d )\n", FtIeLen));
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( Len that doesn't include subelement = %d )\n", RemainLen));
	...
	while (RemainLen > 0) {
		...
		if (subEidPtr->SubElementId == FT_R1KH_ID) {
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("%s : R1KHID length(%d)\n", __func__, subEidPtr->Len));
		} else if (subEidPtr->SubElementId == FT_GTK) {
			*GTKLen = subEidPtr->Len;
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( *GTKLen = %d )\n", *GTKLen));

			if ((*GTKLen >= 15) && (*GTKLen <= 64))
				RTMPMoveMemory(pGTK, subEidPtr->Oct, subEidPtr->Len);
			else {
				*GTKLen = 0;
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("FT- FtIeParse ( Invalid  GTKLen  = %d)\n", *GTKLen));
			}
		} else if (subEidPtr->SubElementId == FT_R0KH_ID) {
			*R0KH_IdLen = subEidPtr->Len;
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( *R0KH_IdLen = %d )\n", *R0KH_IdLen));

			if ((*R0KH_IdLen >= 1) && (*R0KH_IdLen <= 48))
				RTMPMoveMemory(pR0KH_Id, subEidPtr->Oct, subEidPtr->Len);
			else {
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("FT- FtIeParse ( Invalid  R0KH_IdLen = %d )\n", *R0KH_IdLen));
				*R0KH_IdLen = 0;
			}
		}
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("%s done\n", __func__));
}
	...
	IN	PRTMP_ADAPTER	pAd, IN struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("==> FT_CheckForRoaming\n"));
	...
	for (i = 0; i < ScanTab->BssNr; i++) {
		...
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("max_rssi = %d, pBss->Rssi = %d\n", max_rssi, pBss->Rssi));
		/* AP passing all above rules is put into roaming candidate table */
		NdisMoveMemory(&pRoamTab->BssEntry[pRoamTab->BssNr], pBss, sizeof(BSS_ENTRY));
		pRoamTab->BssNr += 1;
	}

	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("<== FT_CheckForRoaming (BssNr=%d)\n", pRoamTab->BssNr));

	if (pRoamTab->BssNr > 0) {
		/* check CntlMachine.CurrState to avoid collision with NDIS SetOID request */
		if (cntl_idle(wdev)) {
			pAd->RalinkCounters.PoorCQIRoamingCount++;
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("MMCHK - Roaming attempt #%ld\n", pAd->RalinkCounters.PoorCQIRoamingCount));
			return TRUE;
		}
	}
	...
	IN	PFT_FTIE_INFO		pFtInfo)
{
	...
	if (pFtInfo->GtkLen < 11) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s : The length is invalid\n", __func__));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("%s : key idx(%d)\n", __func__, pEntry->SecConfig.PairwiseKeyId));
	...
	if (unwrap_len != gtk_len) {
		MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s : The GTK length is unmatched\n", __func__));
		return FALSE;
	}
...
/* #endif */
	...
	if (mode && NdisCmpMemory(mode, "H", 1) == 0) {
		...
		if (value)
			...
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, ("%s: Invalid value.\n", __func__));
	}
