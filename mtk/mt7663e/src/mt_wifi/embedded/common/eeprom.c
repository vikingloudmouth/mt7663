...
#if defined(PRE_CAL_TRX_SET1_SUPPORT) || defined(PRE_CAL_TRX_SET2_SUPPORT) || defined(RLM_CAL_CACHE_SUPPORT) || defined(PRE_CAL_MT7663_SUPPORT)
#ifdef CONFIG_RT_FIRST_CARD
...
static NDIS_STATUS rtmp_ee_bin_init(PRTMP_ADAPTER pAd)
{
...
#ifdef PRE_CAL_MT7663_SUPPORT
	os_alloc_mem(pAd, &pAd->PreCalImageInfo, PRE_CAL_IMAGE_INFO_SIZE);
	os_alloc_mem(pAd, &pAd->PreCalImage, PRE_CAL_TOTAL_SIZE);
	os_alloc_mem(pAd, &pAd->TxDPDImage, DPD_CAL_TOTAL_SIZE);
	NdisZeroMemory(pAd->PreCalImageInfo, PRE_CAL_IMAGE_INFO_SIZE);
	NdisZeroMemory(pAd->PreCalImage, PRE_CAL_TOTAL_SIZE);
	NdisZeroMemory(pAd->TxDPDImage, DPD_CAL_TOTAL_SIZE);
	if (rtmp_cal_load_from_bin(pAd, pAd->PreCalImageInfo, 0, PRE_CAL_IMAGE_INFO_SIZE) == NDIS_STATUS_SUCCESS)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): PreCalImageInfo  Done.\n", __func__));
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): PreCalImageInfo  Fail.\n", __func__));

	if (rtmp_cal_load_from_bin(pAd, pAd->PreCalImage, PRE_CAL_BIN_OFFSET, PRE_CAL_TOTAL_SIZE) == NDIS_STATUS_SUCCESS)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): PreCalImage  Done.\n", __func__));
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	if (rtmp_cal_load_from_bin(pAd, pAd->TxDPDImage, DPD_BIN_OFFSET, DPD_CAL_TOTAL_SIZE) == NDIS_STATUS_SUCCESS)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s(): TxDPDImage  Done.\n", __func__));
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
...
INT Set_DPDBufferWriteBack_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	switch (e2p_mode) {
	case E2P_BIN_MODE:
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Write DPD buffer back to BIN\n"));
		if (rtmp_cal_write_to_bin(pAd, pAd->TxDPDImage, DPD_BIN_OFFSET, DPD_CAL_TOTAL_SIZE) != NDIS_STATUS_SUCCESS) {
	...
	default:
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s::do not support this DPD access mode\n", __func__));
		return FALSE;
	}
	return TRUE;
}

INT Set_PreCalBufferWriteBack_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT e2p_mode = (UCHAR) os_str_tol(arg, 0, 10);
	...
	switch (e2p_mode) {
	case E2P_BIN_MODE:
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Write Pre Cal buffer back to BIN\n"));
		if ((rtmp_cal_write_to_bin(pAd, pAd->PreCalImageInfo, 0, PRE_CAL_IMAGE_INFO_SIZE) != NDIS_STATUS_SUCCESS) ||
			(rtmp_cal_write_to_bin(pAd, pAd->PreCalImage, PRE_CAL_BIN_OFFSET, PRE_CAL_TOTAL_SIZE) != NDIS_STATUS_SUCCESS)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("ERROR !!! Pre CAL  data to BIN write failed \n"));
			return FALSE;
		}
		break;
