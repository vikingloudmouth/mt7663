...
static int CFG80211_OpsVirtualInfChg(IN struct wiphy *pWiphy,
...
static int CFG80211_OpsVirtualInfChg(IN struct wiphy *pWiphy,
				     ...
				     struct vif_params *pParams)
#else
static int CFG80211_OpsVirtualInfChg(IN struct wiphy *pWiphy, IN int IfIndex,
				     IN enum nl80211_iftype Type,
				     IN UINT32 *pFlags,
				     struct vif_params *pParams)
#endif /* LINUX_VERSION_CODE */
{
	...
	if (pParams != NULL) {
		...
		if (((pParams->flags) & NL80211_MNTR_FLAG_FCSFAIL) ==
		...
		if (((pParams->flags) & NL80211_MNTR_FLAG_CONTROL) ==
...
/* Set Channel List for this Scan Action */
	...
	if ((pRequest->n_ssids == 0) || !pRequest->ssids) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("80211> n_ssids == 0 or null ssids Wildcard SSID.\n"));
	...
	IN struct cfg80211_ibss_params	*pParams)
{
	...
	struct ieee80211_channel *pChan = NULL;
...
#if (KERNEL_VERSION(3, 8, 0) <= LINUX_VERSION_CODE)
	pChan = pParams->chandef.chan;
	CFG80211DBG(DBG_LVL_OFF, ("80211> SSID = %s, BI = %d, CH_FIX = %d, Privacy = %d\n",
							  IbssInfo.Ssid, pParams->beacon_interval,
							  pParams->channel_fixed, pParams->privacy));

	if (pChan)
		CFG80211DBG(DBG_LVL_OFF, ("80211> CH = %d\n", pChan->center_freq));

#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0) */
	CFG80211DBG(DBG_LVL_OFF, ("80211> SSID = %s, BI = %d, CH = %d, CH_FIX = %d, Privacy = %d\n",
							  IbssInfo.Ssid, pParams->beacon_interval,
							  pParams->channel->center_freq, pParams->channel_fixed, pParams->privacy));
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0) */
	...
	if (pParams->bssid) {
		CFG80211DBG(DBG_LVL_OFF, ("Join this BSSID: %02X:%02X:%02X:%02X:%02X:%02X\n",
								  PRINT_MAC(pParams->bssid)));
	} else
		CFG80211DBG(DBG_LVL_OFF, ("Can't find any AdHoc, create IBSS\n"));

	/* ibss join */
	RTMP_DRIVER_80211_IBSS_JOIN(pAd, &IbssInfo);

	if (beacon_buf) {
		os_free_mem(beacon_buf);
		beacon_buf = NULL;
	}
...
#endif /* LINUX_VERSION_CODE */
{
	...
	if (pParams->cipher == WLAN_CIPHER_SUITE_AES_CMAC) {
	...
	IN struct cfg80211_connect_params	*pSme)
{
	...
	if (pSme->bssid != NULL) {
		...
		CFG80211DBG(DBG_LVL_OFF, ("CONNECT BSSID IS NULL\n"));
