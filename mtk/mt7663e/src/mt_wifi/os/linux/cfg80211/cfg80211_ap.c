...
static INT CFG80211DRV_UpdateApSettingFromBeacon(PRTMP_ADAPTER pAd, UINT mbss_idx, CMD_RTPRIV_IOCTL_80211_BEACON *pBeacon)
{
	...
	pMbss->CapabilityInfo =	CAP_GENERATE(1, 0, (!IS_CIPHER_NONE(wdev->SecConfig.PairwiseCipher)),
		...
		wdev->bUseShortSlotTime, /*SpectrumMgmt*/FALSE);
...

{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		if (IS_PCI_INF(pAd)) {
			...
			resource_idx = asic_get_resource_idx(pAd, wdev, TX_DATA, 0);
	...
	if (wdev->func_idx < max_v2_bcn_num)
		...
		RT28xx_UpdateBcnAndTimToMcu(pAd, &pAd->ApCfg.MBSSID[apidx].wdev, beacon_len, pAd->ApCfg.MBSSID[apidx].wdev.bcn_buf.TimIELocationInBeacon, PKT_V1_BCN);
#else
	arch_ops->archUpdateBeacon(pAd, &pAd->ApCfg.MBSSID[apidx].wdev, beacon_len, PKT_V2_BCN);
#endif
#else
#ifdef RT_CFG80211_SUPPORT
	arch_ops->archUpdateBeacon(pAd, &pAd->ApCfg.MBSSID[apidx].wdev, beacon_len, PKT_V1_BCN);
#else
...
BOOLEAN CFG80211DRV_OpsBeaconSet(VOID *pAdOrg, VOID *pData)
{
	...
	AsicUpdateBeacon(pAd, wdev, Frame_Len, PKT_V1_BCN);
#else
...
#define MCAST_WCID_TO_REMOVE 0
	...
	AsicUpdateBeacon(pAd, wdev, FrameLen, PKT_V1_BCN);
#else
...
#if defined(RT_CFG80211_SUPPORT) || defined(MT7626)
	...
	UCHAR *pRspIe, UINT32 RspIeLen, UCHAR FlgIsSuccess)
{
	...
	CFG80211OS_P2pClientConnectResultInform(pAd->StaCfg[ifIndex].wdev.if_dev, pBSSID,
