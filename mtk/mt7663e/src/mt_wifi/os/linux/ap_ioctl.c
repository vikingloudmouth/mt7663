...
INT rt28xx_ap_ioctl(void *net_dev_obj, void *data_obj, int cmd) /* snowpin for ap/sta */
{
	...
	if (cmd == SIOCGIWNAME) {
		...
		Status = RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_SIOCGIWNAME, 0, wrqin->u.name, 0);
		return Status;
	} /*- patch for SnapGear */
	...
	case SIOCGIWESSID: { /*Get ESSID */
		...
		Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_AP_SIOCGIWESSID, 0, pIoctlSSID, 0);
		pSsidStr = (PCHAR)pIoctlSSID->pSsidStr;
		erq->length = pIoctlSSID->length;

		if ((Status == NDIS_STATUS_SUCCESS) && (erq->pointer) && (pSsidStr != NULL)) {
	...
	case SIOCGIWAP: { /*get access point MAC addresses */
		...
		Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_AP_SIOCGIWAP, 0,
