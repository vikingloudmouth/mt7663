...
#ifdef IXIA_SUPPORT
#define IS_FORCE_COUNT() (!dectlen_l && !dectlen_m && !dectlen_h)
#define IS_OSEXPECTED_LENGTH(pAd, len) (IS_FORCE_COUNT() || (((len >= (dectlen_l - 4)) && (len <= dectlen_l))\
...
#define THIRD_AP_PROFILE_PATH		"/etc/wireless/mt7663e/mt7663.3.dat"
...
#ifdef IXIA_SUPPORT
	if (IS_OSEXPECTED_LENGTH(pAd, pkt_total_len)) {
		pAd->tr_ctl.tp_dbg.tx_pkt_from_os++;
		pAd->tr_ctl.tp_dbg.txpkt_dct_period++;
		pAd->tr_ctl.tp_dbg.tx_pkt_len = pkt_total_len;
	}
#endif /*IXIA_SUPPORT*/
	if (!pPacket)
		return 0;
...
#ifdef IXIA_SUPPORT
				if (IS_OSEXPECTED_LENGTH(pAd, RTPKT_TO_OSPKT(pPacket)->len))
					pAd->tr_ctl.tp_dbg.TxDropPacket[WIFI_PKT_FWD]++;
#endif /*IXIA_SUPPORT*/
				RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
				return 0;
			}
