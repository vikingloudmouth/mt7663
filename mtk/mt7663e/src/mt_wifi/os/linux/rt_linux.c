...
#define IS_TOOSEXPECTED_LENGTH(len) (((len >= (dectlen_l - 4)) && (len <= dectlen_l)) \
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
				__kernel_write(file_w, msg, strlen(msg),
					       &file_w->f_pos);
...
VOID RtmpOsPktRcvHandle(PNDIS_PACKET pNetPkt)
{
	struct sk_buff *pRxPkt = RTPKT_TO_OSPKT(pNetPkt);
#ifdef IXIA_SUPPORT
	UINT skb_len = pRxPkt->len;
#endif /*IXIA_SUPPORT*/
	netif_rx(pRxPkt);
#ifdef IXIA_SUPPORT
	if(IS_TOOSEXPECTED_LENGTH(skb_len + 18))
		rx_pkt_to_os++;
#endif /*IXIA_SUPPORT*/
}
