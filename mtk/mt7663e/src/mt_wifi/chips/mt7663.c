...
{
	UINT32 i, offset_idx, length;
	UINT32 CentralFreq = 5000 + (Channel * 5);
	INT status = FALSE, ret;
	UINT8 Band = ABAND;
	MT_SWITCH_CHANNEL_CFG ch_cfg;

	if ((Channel >= 1 && Channel <= 13) || (Channel >= 36 && Channel <= 64) || (Channel >= 100 && Channel <= 165))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s : Channel %d\n"
			, __func__, Channel));
	else {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s : UNEXPECTED Channel %d, PLEASE CHECK!! \n"
			, __func__, Channel));
		return status;
	}

	if (((pAd->EEPROMImage[0x52] & (1 << 1)) == 0) && ((pAd->EEPROMImage[0x52] & (1 << 2)) == 0)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	if (Channel < 14) {
		Band = GBAND;
		length = DPD_FLATNESS_2G_CAL_SIZE;
		...
		Band = ABAND;
		length = DPD_FLATNESS_5G_CAL_SIZE;
		CentralFreq = Channel * 5 + 5000;
	}

	/* Find offset based on  CentralFreq uisng DPD+Flatness pre-k table */
	for (i = 0; i < MT7663_DPD_FLATNESS_BW20_FREQ_SIZE; i++) {
		if (MT7663_DPD_FLATNESS_BW20_FREQ[i] == CentralFreq) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("%s: %d is in DPD-Flatness cal table, index = %d\n", __func__, CentralFreq, i));
	...
	if (i == MT7663_DPD_FLATNESS_BW20_FREQ_SIZE) {
		...
		if (length == DPD_FLATNESS_2G_CAL_SIZE) {
			pAd->TxDPDOfst = ((DPD_FLATNESS_5G_CHAN_NUM * DPD_FLATNESS_5G_CAL_SIZE) +
				((offset_idx - DPD_FLATNESS_5G_CHAN_NUM) * length));
		} else {
			pAd->TxDPDOfst = (offset_idx * length);
		}
#else
		pAd->TxDPDOfst = (offset_idx * DPD_FLATNESS_CAL_SIZE);
#endif
		NdisZeroMemory(&ch_cfg, sizeof(ch_cfg));

		ch_cfg.Bw = BW_20;
		ch_cfg.CentralChannel = MT7663_DPD_FLATNESS_BW20_CH[offset_idx];
		ch_cfg.TxStream = 2;
		ch_cfg.RxStream = 3;
		ch_cfg.ControlChannel = MT7663_DPD_FLATNESS_BW20_CH[offset_idx];
		ch_cfg.ControlChannel2 = 0;
		ch_cfg.BandIdx = 0;
		ch_cfg.bScan = 0;
		ch_cfg.Channel_Band = Band;
		MtCmdChannelSwitch(pAd, ch_cfg);
		MtCmdSetTxRxPath(pAd, ch_cfg);
		ret = MtCmdDoCalibration(pAd, RE_CALIBRATION, (1<<28), 0);
...
static UINT32 ICAL_JUST_MERGE[] = {
				...
				/*remove 0x1bb which would not be calibrated in FT*/
				/*0x12A, 0x12B, 0x12C, 0x12D, 0x12E, 0x12F, 0x1BB, 0x1BD,*/
				0x12A, 0x12B, 0x12C, 0x12D, 0x12E, 0x12F, 0x1BD,
				/* WF A-die */
...
static VOID mt7663_fw_prepare(RTMP_ADAPTER *pAd)
{
	...
	if (IS_MT7663_FW_VER_E2(pAd)) {
		...
		ctrl->patch_profile[WM_CPU].source.header_len = sizeof(mt7663_rom_patch_e2);
		ctrl->patch_profile[WM_CPU].source.bin_name = MT7663_ROM_PATCH_BIN_FILE_NAME_E2;
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():using E2 ROM patch as default\n", __func__));
	}
#endif /* NEED_ROM_PATCH */
	...
	if (IS_MT7663_FW_VER_E2(pAd)) {
		...
		ctrl->fw_profile[WM_CPU].source.header_len = sizeof(MT7663_FirmwareImage_E2);
		ctrl->fw_profile[WM_CPU].source.bin_name = MT7663_BIN_FILE_NAME_E2;
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():using E2 firmware as default\n", __func__));
	}
}
