...
INT32 mt_cmd_set_fcc5_min_lpn(RTMP_ADAPTER *pAd, UINT16 min_lpn_update)
{
	...
	SET_CMD_ATTR_RSP_HANDLER(attr, NULL);
#ifdef RT_BIG_ENDIAN
	cmd_set_lpn_update.tag = cpu2le16(cmd_set_lpn_update.tag);
...
INT32 mt_cmd_set_pls_thres_param(RTMP_ADAPTER *pAd, P_CMD_RDM_PULSE_THRESHOLD_UPDATE_T p_pls_threshold)
{
	...
	SET_CMD_ATTR_RSP_HANDLER(attr, NULL);
#ifdef RT_BIG_ENDIAN
	cmd_set_pls_thres_update.tag = cpu2le16(cmd_set_pls_thres_update.tag);
#endif
	MtAndesInitCmdMsg(msg, attr);
...
)
{
	...
	rSetEdccaThreshold.u1BandIdx = BandIdx;
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
...
/*****************************************
    ExT_CID = 0x7A
*****************************************/
INT32 MtCmdSetMinPhyRate(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT8 band)
{
	struct cmd_msg *msg;
	...
	CmdSetMinPhyRate.u1BandIdx = band;

	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s BSSID = %d, DR = %d, MR = %d, DisableCCK = %d band = %d\n",
			 __func__, CmdSetMinPhyRate.ucBssidIdx,
			 CmdSetMinPhyRate.ucMinPhyDataRate,
			 CmdSetMinPhyRate.ucMinPhyMgmtRate,
			 CmdSetMinPhyRate.fgDisableCCKRate,
			 CmdSetMinPhyRate.u1BandIdx));
	...
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)&CmdSetMinPhyRate,
			sizeof(CMD_SET_MIN_PHY_RATE));
